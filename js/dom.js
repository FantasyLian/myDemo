...
function animate(ele, obj, duration, type, fnCallback) {
	var lfcEffect = {
		...
		Bounce: {
			easeIn: function(t, b, c, d) {
				return c - lfcEffect.Bounce.easeOut(d - t, 0, c, d) + b
			},
			easeOut: function(t, b, c, d) {
				if ((t /= d) < (1 / 2.75)) {
			...
			easeInOut: function(t, b, c, d) {
				if (t < d / 2) return lfcEffect.Bounce.easeIn(t * 2, 0, c, d) * .5 + b;
				else return lfcEffect.Bounce.easeOut(t * 2 - d, 0, c, d) * .5 + c * .5 + b
			}
		}
	};
	var fnEffect = lfcEffect.Expo.easeOut;
	if (typeof type == "number") {
		switch (type) {
			case 0:
				break;
			case 1:
				fnEffect = lfcEffect.Linear;
				break;
			case 2:
				fnEffect = lfcEffect.Back.easeOut;
				break;
			case 3:
				fnEffect = lfcEffect.Elastic.easeOut;
				break;
			case 4:
				fnEffect = lfcEffect.Bounce.easeOut;
				break
		}
